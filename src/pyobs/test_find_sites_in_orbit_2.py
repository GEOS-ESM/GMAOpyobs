...
# -*- coding: utf-8 -*-
"""
Feb/1/2025 _ same as _v1 but now it includes
...
def find_nearest_point_haversine(lat_array, lon_array, lat0, lon0):
    ...
    a = np.sin(dlat/2)**2 + np.cos(lat0) * np.cos(lat_array) * np.sin(dlon/2)**2
    
    # Get index of minimum value
    idx = np.argmin(a)  # No need to complete haversine formula if only finding index
  ...
  'GSFC',38.993,-76.84,0.05,datetime(2023, 12, 25, 17, 5),datetime(2023, 12, 25, 19, 5)]\
    ...
    for entry in matching_orbits_list:
         ...
         if orbit_num != current_orbit:
             print(f"\nLoading new file for orbit {orbit_num}: {filename}")
         ...
         # Now perform your operation using the loaded file and other entry data
         site_name = entry[2]
         lat = entry[3]
         lon = entry[4]
         start_time = entry[6]
         ...
         # result = process_data(current_file, lat, lon, start_time, end_time)
         
         # Simulate some operation
         print(f"Operating on data for {site_name} at ({lat}, {lon})")
         #   sys.exit()
            #OK 
            #1) Load lat/long
