...
#tro           'latitude_bounds','longitude_bounds',
      ...
      SCIDATA = ('FinalAerosolAbsOpticalDepth',
               ...
               'FinalAlgorithmFlags',
               'FinalAlgorithmFlagsACA',
               'NormRadiance',
               'Reflectivity',  ##dimension 4172, 450,2
               'Residue',
               'UVAerosolIndex',
...
#       self.Time = array([DATE_START+timedelta(seconds=s) for s in self.Scan_Start_Time])
       self.Time = array([DATE_START+s for s in self.Scan_Start_Time])
...
#      ------------
       ...
       # ---------
       try:
           aod_abs = self.aod_abs[:,I]
       except:
           aod_abs = MISSING * ones(aod.shape) # will compress like a charm
       ...
       f.write('tau_fine', nymd, nhms, 
               binobs3d(self.lon,self.lat,aod_abs,im,jm,MISSING) )
...
if __name__ == "__main__":
    ...
    syn_time = datetime(2022,9,2,10,0,0)
